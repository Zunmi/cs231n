...
def affine_backward(dout, cache):
    ...
    dw = x_row.T @ dout        # (D, N) @ (N, M) = (D, M)
    db = np.sum(dout, axis=0)  # (M,)
    dx_row = dout @ w.T        # (N, M) @ (M, D) = (N, D)
...
def batchnorm_backward(dout, cache):
    ...
    dvar = np.sum(dxhat * xmu * (-0.5) * (var_eps ** (-1.5)), axis=0)
    dmean = np.sum(dxhat * (-std_inv), axis=0) + dvar * np.sum(-2.0 * xmu, axis=0) / N
