...
class FullyConnectedNet(object):
 ...
 # -------------------- forward --------------------
        ...
        # hidden layers: {affine - [norm] - relu - [dropout]} x (L - 1)
        for i in range(1, self.num_layers):
            ...
            if self.normalization == "batchnorm":
                ...
                out, norm_caches[i] = batchnorm_forward(
                    out, gamma, beta, self.bn_params[i - 1]
                )
            elif self.normalization == "layernorm":
                gamma = self.params[f"gamma{i}"]
                beta = self.params[f"beta{i}"]
                out, norm_caches[i] = layernorm_forward(
                    out, gamma, beta, self.bn_params[i - 1]
                )
        ...
        scores, affine_caches[self.num_layers] = affine_forward(out, W_last, b_last)
    ...
    # -------------------- backward --------------------
        ...
        dout, dW, db = affine_backward(dout, affine_caches[self.num_layers])
