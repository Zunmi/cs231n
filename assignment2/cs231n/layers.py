...
def batchnorm_backward(dout, cache):
    ...
    # Step 3: dx_norm -> dcentered and dinv_std
    inv_std = 1.0 / np.sqrt(sample_var + eps)
    x_centered = x - sample_mean
    
    dcentered = dx_norm * inv_std
    dinv_std = np.sum(dx_norm * x_centered, axis=0)

    # Step 4: dinv_std -> dsample_var
    dsample_var = dinv_std * (-0.5) * (sample_var + eps)**(-1.5)

    # Step 5: dsample_var -> dcentered
    dcentered += dsample_var * (2.0 / N) * x_centered

    # Step 6: dcentered -> dx and dsample_mean
    dx = dcentered
    dsample_mean = -np.sum(dcentered, axis=0)

    # Step 7: dsample_mean -> dx
...
def batchnorm_backward_alt(dout, cache):
    ...
    # Derivation of simplified dx:
    # dx = (gamma / (N * std)) * (N * dout - np.sum(dout, axis=0) - x_norm * np.sum(dout * x_norm, axis=0))
    std = np.sqrt(sample_var + eps)
    dx = (gamma / (N * std)) * (N * dout - np.sum(dout, axis=0) - x_norm * np.sum(dout * x_norm, axis=0))
    ###########################################################################
    #                             END OF YOUR CODE                            #
...
def layernorm_backward(dout, cache):
    ...
    dcentered += dvar * (2.0 / D) * x_centered
    dx = dcentered
    
    dmean = -np.sum(dcentered, axis=1, keepdims=True)
    dx += (1.0 / D) * dmean
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
